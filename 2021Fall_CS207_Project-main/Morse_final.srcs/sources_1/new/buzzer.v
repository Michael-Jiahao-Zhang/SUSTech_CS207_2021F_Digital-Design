`timescale 1ns / 1ps

module buzzer(
       input clk,rst,modulee,key_on,
       input[3:0]key,
       output reg beep
    );
    reg [35:0] cnt;
reg [5:0]mode=17;
parameter do_low = 191110;
parameter re_low = 170259;
parameter me_low = 151685;
parameter fa_low = 143172;
parameter so_low = 127554;
parameter la_low = 113636;
parameter si_low = 101239;

parameter do = 93941;
parameter re = 85136;
parameter me = 75838;
parameter fa = 71582;
parameter so = 63776;
parameter la = 56818;
parameter si = 50618;

parameter do_high = 47778;
parameter re_high = 42567;
parameter me_high = 37921;
parameter fa_high = 36498;
parameter so_high = 31888;
parameter la_high = 28409;
parameter si_high = 25309;

parameter beat = 40 * 500000;
parameter gap =  10 * 500000;
parameter index_period = beat + gap;

parameter silence = beat<<9;

/*
0 - silence
1 - 7 low
8 - 14 meidum
15 - 21 high
*/
parameter JiangNan = 1660';
parameter HappyBirthday = 130'b0111110000011111000110010000000000001101011100111110001100110110001111011111000001100011010110001110011100111101100011010010100101;
parameter MerryChristmas = 335';

parameter JN_length = 332;
parameter HB_length = 26;
parameter MC_length = 67;

reg[29:0] freq =  beat;

reg[2000:0] melody = 0;
integer melody_length = 0;

integer frequency_count = 0;      // count1 control frequency
integer index_count = 0;      // count2 control beat;

integer index = 0;       // index control the location music playing

reg [0:0] isSilence = 0; 
reg [0:0] isEnd = 0;
reg [0:0] isPeriodic = 0;

reg[4:0] last_mode = 0;





always @* begin

if(isSilence || isEnd)
freq = silence;
else
case(melody[index * 5 +4 -:5])
5'd0 : freq = silence;
5'd1 : freq = do_low;
5'd2 : freq = re_low;
5'd3 : freq = me_low;
5'd4 : freq = fa_low;
5'd5 : freq = so_low;
5'd6 : freq = la_low;
5'd7 : freq = si_low;
5'd8 : freq = do;
5'd9 : freq = re;
5'd10 : freq = me;
5'd11: freq = fa;
5'd12 : freq = so;
5'd13 : freq = la;
5'd14 : freq = si;
5'd15 : freq = do_high;
5'd16 : freq = re_high;
5'd17 : freq = me_high;
5'd18 : freq = fa_high;
5'd19 : freq = so_high;
5'd20 : freq = la_high;
5'd21 : freq = si_high;
default : freq = silence;
endcase
end

    //分频计时，总共二十秒，在key_on为0时，令cnt固定为0
    always@(posedge clk, posedge rst)//可能有问题
    begin
        if(rst)
        begin 
            cnt <= 1'b0;
        end
        
        else if(key_on==0)
        begin
            cnt<=1'b0;
         end
        else if(cnt != 35'd1_999_999_999)    //20s
            cnt <= cnt + 1'b1;
            
        else 
            cnt <= 1'b0;        //清空
    end
     /*
    //蜂鸣器播放核心代码，在读入key、modulee、key_on之后判断是否进入播放部分，计时器在对应的区间内，在每过一段时间对beep取反从而获取
    人耳能听到的声音
    以下为播放key==1,模式为短模式时的代码
    */
    always@(posedge clk)
    if(key==1&&modulee==0&&key_on==1)
    begin 
        //第一个频带（第一声 短）
        if(cnt < 35'd50_000_000)
        begin
            if(cnt%27'd49_999==0)
                beep <= ~beep;
        end
        //第二个频带（第二声 长）
        else if(cnt > 35'd100_000_000 && cnt < 35'd200_000_000)
        begin
            if(cnt%27'd49_999==0)
                beep <= ~beep;
        end
        //第三个频带（第三声 长）
        else if(cnt > 35'd250_000_000 && cnt < 35'd350_000_000)
        begin
            if(cnt%27'd49_999==0)
                beep <= ~beep;
        end
        //第四个频带（第四声 长）
        else if(cnt > 35'd400_000_000 && cnt < 35'd500_000_000)
        begin
            if(cnt%27'd49_999==0)
                beep <= ~beep;
        end
        //（第五声 长）
        else if(cnt > 35'd550_000_000 && cnt < 35'd650_000_000)
        begin
            if(cnt%27'd49_999==0)
                beep <= ~beep;
        end
    end 
    else if(key==2&&modulee==0&&key_on==1)
    begin
        //第一个频带（第一声 短）
            if(cnt < 35'd50_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第二个频带（第二声 短）
            else if(cnt > 35'd100_000_000 && cnt < 35'd150_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第三个频带（第三声 长）
            else if(cnt > 35'd200_000_000 && cnt < 35'd300_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第四个频带（第四声 长）
            else if(cnt > 35'd350_000_000 && cnt < 35'd450_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //（第五声 长）
            else if(cnt > 35'd500_000_000 && cnt < 35'd600_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
    end
    else if(key==3&&modulee==0&&key_on==1)
        begin
            //第一个频带（第一声 短）
                if(cnt < 35'd50_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
                //第二个频带（第二声 短）
                else if(cnt > 35'd100_000_000 && cnt < 35'd150_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
                //第三个频带（第三声  短）
                else if(cnt > 35'd200_000_000 && cnt < 35'd250_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
                //第四个频带（第四声 长）
                else if(cnt > 35'd300_000_000 && cnt < 35'd400_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
                //（第五声 长）
                else if(cnt > 35'd450_000_000 && cnt < 35'd550_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
        end
    else if(key==4&&modulee==0&&key_on==1)
            begin
                //第一个频带（第一声 短）
                    if(cnt < 35'd50_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
                    //第二个频带（第二声 短）
                    else if(cnt > 35'd100_000_000 && cnt < 35'd150_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
                    //第三个频带（第三声  短）
                    else if(cnt > 35'd200_000_000 && cnt < 35'd250_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
                    //第四个频带（第四声 短）
                    else if(cnt > 35'd300_000_000 && cnt < 35'd350_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
                    //（第五声 长）
                    else if(cnt > 35'd400_000_000 && cnt < 35'd500_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
            end
    else if(key==5&&modulee==0&&key_on==1)
                    begin
                        //第一个频带（第一声 短）
                            if(cnt < 35'd50_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                            //第二个频带（第二声 短）
                            else if(cnt > 35'd100_000_000 && cnt < 35'd150_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                            //第三个频带（第三声  短）
                            else if(cnt > 35'd200_000_000 && cnt < 35'd250_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                            //第四个频带（第四声 短）
                            else if(cnt > 35'd300_000_000 && cnt < 35'd350_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                            //（第五声 短）
                            else if(cnt > 35'd400_000_000 && cnt < 35'd450_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                    end
    else if(key==6&&modulee==0&&key_on==1)
                    begin 
                        //第一个频带（第一声 长）
                        if(cnt < 35'd100_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                        //第二个频带（第二声 短）
                        else if(cnt > 35'd150_000_000 && cnt < 35'd200_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                        //第三个频带（第三声 短）
                        else if(cnt > 35'd250_000_000 && cnt < 35'd300_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                        //第四个频带（第四声 短）
                        else if(cnt > 35'd350_000_000 && cnt < 35'd400_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                        //（第五声 短）
                        else if(cnt > 35'd450_000_000 && cnt < 35'd500_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                    end 
else if(key==7&&modulee==0&&key_on==1)
                        begin
                            //第一个频带（第一声 长）
                                if(cnt < 35'd100_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                                //第二个频带（第二声 长）
                                else if(cnt > 35'd150_000_000 && cnt < 35'd250_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                                //第三个频带（第三声 短）
                                else if(cnt > 35'd300_000_000 && cnt < 35'd350_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                                //第四个频带（第四声 短）
                                else if(cnt > 35'd400_000_000 && cnt < 35'd450_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                                //（第五声 短）
                                else if(cnt > 35'd500_000_000 && cnt < 35'd550_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                        end
else if(key==8&&modulee==0&&key_on==1)
                           begin 
                               //第一个频带（第一声 长）
                               if(cnt < 35'd100_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第二个频带（第二声 长）
                               else if(cnt > 35'd150_000_000 && cnt < 35'd250_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第三个频带（第三声 长）
                               else if(cnt > 35'd300_000_000 && cnt < 35'd400_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第四个频带（第四声 短）
                               else if(cnt > 35'd450_000_000 && cnt < 35'd500_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //（第五声 短）
                               else if(cnt > 35'd550_000_000 && cnt < 35'd600_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                           end
else if(key==9&&modulee==0&&key_on==1)                           
                            begin 
                               //第一个频带（第一声 长）
                               if(cnt < 35'd100_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第二个频带（第二声 长）
                               else if(cnt > 35'd150_000_000 && cnt < 35'd250_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第三个频带（第三声 长）
                               else if(cnt > 35'd300_000_000 && cnt < 35'd400_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第四个频带（第四声 长）
                               else if(cnt > 35'd450_000_000 && cnt < 35'd550_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //（第五声 短）
                               else if(cnt > 35'd600_000_000 && cnt < 35'd650_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                           end
 else if(key==0&&modulee==0&&key_on==1)                             
 begin 
                               //第一个频带（第一声 长）
                               if(cnt < 35'd100_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第二个频带（第二声 长）
                               else if(cnt > 35'd150_000_000 && cnt < 35'd250_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第三个频带（第三声 长）
                               else if(cnt > 35'd300_000_000 && cnt < 35'd400_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第四个频带（第四声 长）
                               else if(cnt > 35'd450_000_000 && cnt < 35'd550_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //（第五声 长）
                               else if(cnt > 35'd600_000_000 && cnt < 35'd700_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                           end
                           
    else if(key==1&&modulee==1&&key_on==1)
    begin
        //第一个频带（第一声 短）
            if(cnt < 35'd100_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第二个频带（第二声 长）
            else if(cnt > 35'd200_000_000 && cnt < 35'd400_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第三个频带（第三声 长）
            else if(cnt > 35'd500_000_000 && cnt < 35'd700_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第四个频带（第四声 长）
            else if(cnt > 35'd800_000_000 && cnt < 35'd1000_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //（第五声 长）
            else if(cnt > 35'd1100_000_000 && cnt < 35'd1400_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
    end
    else if(key==2&&modulee==1&&key_on==1)
    begin
        //第一个频带（第一声 短）
            if(cnt < 35'd100_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第二个频带（第二声 短）
            else if(cnt > 35'd200_000_000 && cnt < 35'd300_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第三个频带（第三声 长）
            else if(cnt > 35'd400_000_000 && cnt < 35'd600_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //第四个频带（第四声 长）
            else if(cnt > 35'd700_000_000 && cnt < 35'd900_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
            //（第五声 长）
            else if(cnt > 35'd1000_000_000 && cnt < 35'd1200_000_000)
            begin
                if(cnt%27'd49_999==0)
                    beep <= ~beep;
            end
    end
    else if(key==3&&modulee==1&&key_on==1)
        begin
            //第一个频带（第一声 短）
                if(cnt < 35'd100_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
                //第二个频带（第二声 短）
                else if(cnt > 35'd200_000_000 && cnt < 35'd300_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
                //第三个频带（第三声  短）
                else if(cnt > 35'd400_000_000 && cnt < 35'd500_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
                //第四个频带（第四声 长）
                else if(cnt > 35'd600_000_000 && cnt < 35'd800_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
                //（第五声 长）
                else if(cnt > 35'd900_000_000 && cnt < 35'd1100_000_000)
                begin
                    if(cnt%27'd49_999==0)
                        beep <= ~beep;
                end
        end
    else if(key==4&&modulee==1&&key_on==1)
            begin
                //第一个频带（第一声 短）
                    if(cnt < 35'd100_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
                    //第二个频带（第二声 短）
                    else if(cnt > 35'd200_000_000 && cnt < 35'd300_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
                    //第三个频带（第三声  短）
                    else if(cnt > 35'd400_000_000 && cnt < 35'd500_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
                    //第四个频带（第四声 短）
                    else if(cnt > 35'd600_000_000 && cnt < 35'd700_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
                    //（第五声 长）
                    else if(cnt > 35'd800_000_000 && cnt < 35'd1000_000_000)
                    begin
                        if(cnt%27'd49_999==0)
                            beep <= ~beep;
                    end
            end
    else if(key==5&&modulee==1&&key_on==1)
                    begin
                        //第一个频带（第一声 短）
                            if(cnt < 35'd100_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                            //第二个频带（第二声 短）
                            else if(cnt > 35'd200_000_000 && cnt < 35'd300_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                            //第三个频带（第三声  短）
                            else if(cnt > 35'd400_000_000 && cnt < 35'd500_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                            //第四个频带（第四声 短）
                            else if(cnt > 35'd600_000_000 && cnt < 35'd700_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                            //（第五声 短）
                            else if(cnt > 35'd800_000_000 && cnt < 35'd900_000_000)
                            begin
                                if(cnt%27'd49_999==0)
                                    beep <= ~beep;
                            end
                    end
    else if(key==6&&modulee==1&&key_on==1)
                    begin 
                        //第一个频带（第一声 长）
                        if(cnt < 35'd200_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                        //第二个频带（第二声 短）
                        else if(cnt > 35'd300_000_000 && cnt < 35'd400_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                        //第三个频带（第三声 短）
                        else if(cnt > 35'd500_000_000 && cnt < 35'd600_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                        //第四个频带（第四声 短）
                        else if(cnt > 35'd700_000_000 && cnt < 35'd800_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                        //（第五声 短）
                        else if(cnt > 35'd900_000_000 && cnt < 35'd1000_000_000)
                        begin
                            if(cnt%27'd49_999==0)
                                beep <= ~beep;
                        end
                    end 
                else if(key==7&&modulee==1&&key_on==1)
                        begin
                            //第一个频带（第一声 长）
                                if(cnt < 35'd200_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                                //第二个频带（第二声 长）
                                else if(cnt > 35'd300_000_000 && cnt < 35'd500_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                                //第三个频带（第三声 短）
                                else if(cnt > 35'd600_000_000 && cnt < 35'd700_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                                //第四个频带（第四声 短）
                                else if(cnt > 35'd800_000_000 && cnt < 35'd900_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                                //（第五声 短）
                                else if(cnt > 35'd1000_000_000 && cnt < 35'd1100_000_000)
                                begin
                                    if(cnt%27'd49_999==0)
                                        beep <= ~beep;
                                end
                        end
                        else if(key==8&&modulee==1&&key_on==1)
                           begin 
                               //第一个频带（第一声 长）
                               if(cnt < 35'd200_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第二个频带（第二声 长）
                               else if(cnt > 35'd300_000_000 && cnt < 35'd500_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第三个频带（第三声 长）
                               else if(cnt > 35'd600_000_000 && cnt < 35'd800_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第四个频带（第四声 短）
                               else if(cnt > 35'd900_000_000 && cnt < 35'd1000_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //（第五声 短）
                               else if(cnt > 35'd1100_000_000 && cnt < 35'd1200_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                           end
                        else if(key==9&&modulee==1&&key_on==1)                           
                            begin 
                               //第一个频带（第一声 长）
                               if(cnt < 35'd200_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第二个频带（第二声 长）
                               else if(cnt > 35'd300_000_000 && cnt < 35'd500_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第三个频带（第三声 长）
                               else if(cnt > 35'd600_000_000 && cnt < 35'd800_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第四个频带（第四声 长）
                               else if(cnt > 35'd900_000_000 && cnt < 35'd1100_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //（第五声 短）
                               else if(cnt > 35'd1200_000_000 && cnt < 35'd1300_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                           end
                         else if(key==0&&modulee==1&&key_on==1)                             
                         begin 
                               //第一个频带（第一声 长）
                               if(cnt < 35'd200_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第二个频带（第二声 长）
                               else if(cnt > 35'd300_000_000 && cnt < 35'd500_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第三个频带（第三声 长）
                               else if(cnt > 35'd500_000_000 && cnt < 35'd700_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //第四个频带（第四声 长）
                               else if(cnt > 35'd800_000_000 && cnt < 35'd1000_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                               //（第五声 长）
                               else if(cnt > 35'd1200_000_000 && cnt < 35'd1400_000_000)
                               begin
                                   if(cnt%27'd49_999==0)
                                       beep <= ~beep;
                               end
                           end
                           else if(key==11&&modulee==0&&key_on==1)
                           begin
                                       if(mode != last_mode) begin
                               last_mode = mode;
                               isEnd = 0;
                               index = 0;
                               index_count = 0;
                               
                               if(mode >= 17) isPeriodic = 1;
                               else begin isPeriodic = 0; melody_length = 1; end
                               
                               if(mode == 17) melody_length = MC_length;
                               if(mode == 18) melody_length = HB_length;
                               if(mode == 19) melody_length = JN_length;
                              
                               case(mode)
                                   17: melody = MerryChristmas;
                                   18: melody = HappyBirthday;
                                   19: melody = JiangNan;
                                   default : melody = mode;
                               endcase
                              
                           end
                       
                       
                           if(frequency_count >= freq) begin
                               frequency_count = 0;
                               beep = ~beep;
                           end
                           else frequency_count = frequency_count + 1;
                           
                           if(index_count <= gap) begin
                               isSilence = 1;
                           end
                           
                           if(gap < index_count && index_count <= index_period) begin
                               isSilence = 0;
                           end
                           
                           if(index_count > index_period) begin
                               index_count = 0;
                               index = index + 1;
                               if(index > melody_length && isPeriodic) begin
                                   isEnd = 0;
                                   index = 0;
                               end
                               if(index > melody_length && !isPeriodic) begin
                                   index = 0;
                                   isEnd = 1;
                               end
                           end
                           
                           index_count = index_count + 1;
                           end
    endmodule